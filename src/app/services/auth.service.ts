...
export class AuthService {
    ...
    private fetchUserInfoArc() {
        if (this.token != null) {   
           
            this.http.get(this.oAuthUserUrl + "?client_id="+this.clientId + "&grant_type=authorization_code&client_secret="+this.clientSecret + "&code="+ this.token+"&redirect_uri=" +this.oAuthCallbackUrl)
                .map(res => res.json())
                .subscribe(info => {
                    this.userInfo = info;
                }, err => {
